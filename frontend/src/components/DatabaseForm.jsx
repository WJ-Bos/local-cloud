...
const MEMORY_TIERS = [
  { id: 'none', label: 'No Limit',  mb: null,  description: 'Unconstrained (current default)' },
  { id: 'XS',   label: 'Micro',     mb: 128,   description: '128 MB' },
  { id: 'S',    label: 'Small',     mb: 256,   description: '256 MB' },
  { id: 'M',    label: 'Medium',    mb: 512,   description: '512 MB — recommended' },
  { id: 'L',    label: 'Large',     mb: 1024,  description: '1 GB' },
];

const DatabaseForm = ({ onSubmit, onClose }) => {
  const [databaseName, setDatabaseName] = useState('');
  const [databaseType, setDatabaseType] = useState('POSTGRESQL');
  const [version, setVersion] = useState(DATABASE_TYPES.find(t => t.id === 'POSTGRESQL').defaultVersion);
  const [memoryTier, setMemoryTier] = useState('none');
  ...
  const handleTypeChange = (type) => {
    ...
    const typeInfo = DATABASE_TYPES.find(t => t.id === type);
    setVersion(typeInfo.defaultVersion);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedTier = MEMORY_TIERS.find(t => t.id === memoryTier);

    const payload = {
      name: databaseName,
      type: databaseType,
      version,
      ...(selectedTier?.mb != null && { memoryMb: selectedTier.mb }),
    ...
    try {
      ...
      setVersion(DATABASE_TYPES.find(t => t.id === 'POSTGRESQL').defaultVersion);
      setMemoryTier('none');
      setPort('');
      setUseAutoPort(true);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Engine selector */}
      <DatabaseTypeSelector selectedType={databaseType} onTypeSelect={handleTypeChange} />

      {/* Version + Memory — side by side */}
      <div className="grid grid-cols-2 gap-4">

        {/* Version picker */}
        <div className="space-y-2">
          <label htmlFor="version" className="block text-[10px] font-semibold text-primary-gray-600 uppercase tracking-widest">
          ...
          <div className="relative">
            ...
            >
              {selectedTypeInfo?.versions.map((v) => (
                <option key={v} value={v} className="bg-[#0f1024] text-white">
                  {v}{v === selectedTypeInfo.defaultVersion ? ' (default)' : ''}
                </option>
              ))}
            </select>
        ...
        {/* Memory tier */}
        <div className="space-y-2">
          <label htmlFor="memoryTier" className="block text-[10px] font-semibold text-primary-gray-600 uppercase tracking-widest">
            Memory Limit
          </label>
          <div className="relative">
            <select
              ...
              className="w-full island-inset rounded-xl px-4 py-2.5 text-sm text-white appearance-none cursor-pointer focus:outline-none pr-9"
            >
              {MEMORY_TIERS.map((t) => (
      ...
      <div className="island-inset rounded-xl px-4 py-3.5">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div>
            <div className="text-[10px] text-primary-gray-700 mb-1">Version</div>
            <div className="text-sm font-mono font-medium text-white">{version}</div>
          </div>
          <div>
            <div className="text-[10px] text-primary-gray-700 mb-1">Memory</div>
            <div className="text-sm font-mono font-medium text-white">
              {MEMORY_TIERS.find(t => t.id === memoryTier)?.mb != null
                ? `${MEMORY_TIERS.find(t => t.id === memoryTier).mb} MB`
                : '—'}
            </div>
          </div>
          <div>
