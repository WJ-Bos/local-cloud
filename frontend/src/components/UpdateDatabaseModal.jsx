...
const MEMORY_TIERS = [
  { id: 'none', label: 'No Limit',  mb: null,  description: 'Unconstrained' },
...
const mbToTierId = (mb) => {
  if (mb == null) return 'none';
  const match = MEMORY_TIERS.find(t => t.mb === mb);
...
const UpdateDatabaseModal = ({ isOpen, onClose, database, onSubmit, onStop }) => {
  ...
  useEffect(() => {
    if (database) {
      ...
      setMemoryTier(mbToTierId(database.memoryMb));
      setError('');
    }
  }, [database, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    const newMemoryMb = selectedTier?.mb ?? null;

    if (newName === database.name && portNum === database.port && newMemoryMb === (database.memoryMb ?? null)) {
    ...
    const payload = {
      ...
      newName: newName,
      port: portNum,
      ...(newMemoryMb != null && { memoryMb: newMemoryMb }),
    };
  ...
  const currentMemoryTierId = mbToTierId(database.memoryMb);
  const hasMemoryChanged = memoryTier !== currentMemoryTierId;
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white/[0.02] border border-white/[0.06] rounded-lg p-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div>
              <div className="text-xs text-primary-gray-600 mb-1">Memory</div>
              <div className="text-sm font-mono font-medium text-white">
                {database.memoryMb != null ? `${database.memoryMb} MB` : '—'}
              </div>
            </div>
            <div>
        ...
        <div className={`space-y-4 transition-opacity duration-200 ${isLocked ? 'opacity-35 pointer-events-none select-none' : ''}`}>
          ...
          {/* Memory limit */}
          <div className="space-y-1.5">
            <label htmlFor="memoryTier" className="block text-[10px] font-semibold text-primary-gray-600 uppercase tracking-wider">
              Memory Limit
            </label>
            <div className="relative">
              <select
                ...
                disabled={isLocked}
                className="w-full px-4 py-2.5 bg-[#0A0A0B] text-white border border-white/[0.08] rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 appearance-none cursor-pointer transition-all text-sm pr-9 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {MEMORY_TIERS.map((t) => (
          ...
          {/* Changed preview */}
          {(hasNameChanged || hasPortChanged || hasMemoryChanged) && (
            <div className="bg-emerald-500/[0.05] border border-emerald-500/20 rounded-lg p-3">
              ...
              <div className="flex gap-4">
                ...
                {hasMemoryChanged && (
                  <div>
                    <div className="text-[10px] text-primary-gray-600 mb-0.5">Memory</div>
