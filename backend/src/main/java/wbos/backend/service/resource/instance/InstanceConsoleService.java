...
public class InstanceConsoleService {
    ...
    public ConsoleLogsResponse getLogs(Long instanceId, int tail, String filter) {
        Instance instance = requireRunning(instanceId);
        ...
        try (DockerClient docker = dockerClientFactory.createClient()) {
            ...
            log.info("Fetched {} log lines for instance {} (filter={})", result.size(), instanceId, filter);
            return ConsoleLogsResponse.of(result);

        } catch (Exception e) {
            log.error("Failed to get logs for instance {}: {}", instanceId, e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                    "Failed to retrieve logs: " + e.getMessage());
        }
    }

    public String getInspect(Long instanceId) {
        Instance instance = requireRunning(instanceId);

        try (DockerClient docker = dockerClientFactory.createClient()) {
            var inspect = docker.inspectContainerCmd(instance.getContainerId()).exec();
            ...
            log.error("Failed to inspect instance {}: {}", instanceId, e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                    "Failed to inspect container: " + e.getMessage());
        }
    }

    private Instance requireRunning(Long instanceId) {
        ...
        if (instance.getContainerId() == null || instance.getContainerId().isBlank()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "No container ID for instance: " + instance.getName());
        }
